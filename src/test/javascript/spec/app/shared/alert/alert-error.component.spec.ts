...
import { SldsTestModule } from '../../../test.module';
  ...
  describe('Alert Error Component', () => {
    ...
    beforeEach(async(() => {
      TestBed.configureTestingModule({
        imports: [SldsTestModule, TranslateModule.forRoot()],
    ...
    describe('Error Handling', () => {
      it('Should display an alert on status 0', () => {
        // GIVEN
        eventManager.broadcast({ name: 'sldsApp.httpError', content: { status: 0 } });
        // THEN
      ...
      it('Should display an alert on status 404', () => {
        // GIVEN
        eventManager.broadcast({ name: 'sldsApp.httpError', content: { status: 404 } });
        // THEN
        expect(comp.alerts.length).toBe(1);
      ...
      it('Should display an alert on generic error', () => {
        // GIVEN
        eventManager.broadcast({ name: 'sldsApp.httpError', content: { error: { message: 'Error Message' } } });
        eventManager.broadcast({ name: 'sldsApp.httpError', content: { error: 'Second Error Message' } });
        // THEN
      ...
      it('Should display an alert on status 400 for generic error', () => {
        ...
        eventManager.broadcast({ name: 'sldsApp.httpError', content: response });
        // THEN
        expect(comp.alerts.length).toBe(1);
